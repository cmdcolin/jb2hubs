...
function createTrackConfiguration({ track, trackName, trackDb, trackDbUrl, sequenceAdapter, assemblyName }) {
	...
	return conf ? {
		...
		category: [group, ...extractParentTracks(trackName, trackDb).map((p) => trackDb.data[p.name]?.data.shortLabel)].filter((f) => !!f).map((f) => categoryMap[f] ?? f),
		...conf,
		name: [conf.name].join(" - ")
	} : void 0;
}
...
function makeTrackConfigSub({ track, trackDbUrl, trackDb, sequenceAdapter }) {
	...
	else if (baseTrackType.startsWith("big")) return {
		type: "FeatureTrack",
		adapter: {
			type: "BigBedAdapter",
			uri: bigDataUrl
...
function generateHubTracks({ trackDb, trackDbUrl, assemblyName, sequenceAdapter }) {
	return Object.entries(trackDb.data).map(([trackName, track]) => isMetaTrack(track) ? void 0 : createTrackConfiguration({
		track,
		trackName,
		trackDb,
		trackDbUrl,
...
async function generateJBrowseConfigForAssemblyHub({ hubFileText, trackDbUrl }) {
	if (hubFileText.includes("useOneFile on")) {
		...
		const asm = {
			...
			...chromAliasBb && await hasAliases(resolve(chromAliasBb.replace(".bb", ".txt"), trackDbUrl)) ? { refNameAliases: { adapter: {
...
function extractStats(xmlString) {
	const stats = {};
	const statsRegex = /<Stat category="([^"]+)" sequence_tag="([^"]+)">([^<]+)<\/Stat>/g;
	let match;
	while ((match = statsRegex.exec(xmlString)) !== null) stats[match[1]] = match[3];
	return stats;
}
...
//#endregion
export { categoryMap, dedupe, generateHubTracks, generateJBrowseConfigForAssemblyHub, hubCategories, makeLoc, makeLoc2, makeLocAlt, myfetch, myfetchjson, myfetchtext, myjsonfetch, notEmpty, parseAssemblyEntry, readJSON, resolve, writeJSON };