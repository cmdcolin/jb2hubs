...
rclone sync -c -v --exclude "*.hash" --exclude "*.xxh" --exclude "*meta.json" --exclude "*ncbi.json" --exclude "*.notfound" --exclude "image.json" --exclude "hub.txt" hubs jbrowse-data:jbrowse.org/hubs/genark --s3-storage-class INTELLIGENT_TIERING --checkers 20
